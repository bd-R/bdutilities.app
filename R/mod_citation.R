...
mod_citation_server <- function(input, output, session, package) {
  ...
  output$citationsUI <- renderUI({
    withProgress(message = "Generating Citations", {
      components <- list()
      components[[1]] <- tagList(h3("R"),
                                 p(suppressWarnings(format(
                                   citation(), style = "text"
                                 )[1])))
      components[[2]] <- tagList(h3(package),
                                 p(suppressWarnings(format(
                                   citation(package), style = "text"
                                 )[1])))
      ...
      if (input$citation_level == 1) {
        ...
        for (ind in 1:15) {
          components[[ind + 2]] <- tagList(h3(dep[ind]),
                                           p(suppressWarnings(
                                             format(citation(dep[ind]), style = "text")[1]
                                           )))
        }
      } else {
        dep <- gtools::getDependencies(package, available = F)
  ...
  output$download_bib <- downloadHandler(
    ...
    content = function(con) {
      withProgress(message = "Downloading Citations", {
        if (input$citation_level == 1) {
          cont <- c("base", package)
        } else if (input$citation_level == 2) {
          cont <-
            c("base", package, rev(gtools::getDependencies(package, available = F))[1:15])
        } else {
          cont <-
            c("base", package, rev(gtools::getDependencies(package, available = F)))
        }
